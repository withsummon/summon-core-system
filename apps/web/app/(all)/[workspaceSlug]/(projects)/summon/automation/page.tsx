  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
        <SummonMetric label="Templates" value={data?.templates.length ?? 0} detail="Deterministic generators" />
        <SummonMetric label="Generated jobs" value={jobs.length} detail="Automation history" />
        <SummonMetric
          label="Completed"
          value={jobs.filter(({ status }) => status === "completed").length}
          detail="Saved to Plane Pages"
        />
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {data.templates.map((item) => (
            <button
              ...
              className="shadow-sm rounded-xl border border-subtle bg-surface-1 p-4 text-left hover:border-accent-strong"
            >
              <p className="text-sm font-semibold text-primary">{item.name}</p>
              <p className="text-xs mt-1 line-clamp-2 text-secondary">{item.description}</p>
            </button>
          ))}
        </div>
      ) : null}
      <div className="grid items-start gap-4 lg:grid-cols-[22rem_minmax(0,1fr)]">
        <SummonCard>
          <h2 className="text-sm font-semibold text-primary">Create document</h2>
